...
import NumberLine2 from "../components/NumberLine2";
...
function Subtraction() {
  ...
  function getCalculation() {
    ...
    if (hard || extraHard) {
      ...
      do {
        firstNumber = 18 - Math.floor(Math.random() * 7); // Generate random number between 3 and 20 (to include 10)
        secondNumber = 2 + Math.floor(Math.random() * 14); // Generate random number between 3 and 20 (to include 10)
      } while (
        secondNumber >= firstNumber || // First number should be higher than the second number
        firstNumber > 18 || // First number should not exceed 18
        firstNumber - secondNumber >= 10 || // Sum should not be exactly 10
        firstNumber < 11 || // First number should not be higher than 9
  ...
  function hardCalculations() {
    ...
    do {
      firstNumber = 18 - Math.floor(Math.random() * 7); // Generate random number between 3 and 15
      secondNumber = 2 + Math.floor(Math.random() * 14); // Generate random number between 3 and 15
    } while (
      firstNumber > 20 || // Sum should not exceed 20
      firstNumber - secondNumber >= 10 || // Sum should not be exactly 10 (or higher)
      firstNumber < 11 || // First number should not be higher than 9
      firstNumber - secondNumber <= 0 || // sum should always be above 1
  ...
  function handleCheck() {
    ...
    if (entry1 !== "") {
      if (leftNumber - 10 === Number(entry1)) {
        setRight1(true);
      } else {
        setWrong1(true);
        setNoHelp1(false);
    ...
    if (entry2 !== "") {
      if (rightNumber - (leftNumber - 10) === Number(entry2)) {
        setRight2(true);
      } else {
        setWrong2(true);
        setNoHelp2(false);
  ...
  return (
    <div className="flex flex-col items-center justify-center relative h-screen w-screen">
      ...
      />
      {feedBack ? (
        <FeedBack tips={tips} finalRight={finalRight} finalWrong={finalWrong} />
      ) : (
        <ShowStartingPoints
          click1={click1}
          click2={click2}
